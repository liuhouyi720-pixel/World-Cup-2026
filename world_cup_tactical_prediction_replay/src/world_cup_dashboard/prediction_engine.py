...
class PredictionEngine:
    ...
    def predict(self,m,form,mode="backtest",use_form=True):
        ...
        available={k:v is not None for k,v in c.items()}; limits=["历史代理数据，不是 2026 年赛事级事件数据。"]
        if xt is None: limits.append("xT 档案缺失，相关权重已移除并重归一化。")
        if tactical is None: limits.append("阵型对位数据缺失，相关权重已移除并重归一化。")
        if recent is None: limits.append("本届赛事赛前状态尚不可用。")
        if sample and sample<15: limits.append(f"阵型对位样本较小（{sample} 场）。")
        conf="低" if len(limits)>2 or abs(adv)<.04 else "中等"
